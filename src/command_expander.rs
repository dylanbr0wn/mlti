...
/// Order doesn't matter — each prefix is matched with `starts_with`, so
/// "pnpm:" won't false-match "npm:" and "node:" won't match mid-string.
...
mod tests {
  ...
  fn parse_wildcard_unknown_prefix_returns_none() {
    assert_eq!(parse_wildcard("cargo:build:*"), None);
  }
  ...
  #[test]
  fn expand_shortcut_node() {
    let (cmd, name) = expand_shortcut("node:test").unwrap();
    assert_eq!(cmd, "node --run test");
    assert_eq!(name, "test");
  }

  #[test]
  fn expand_shortcut_deno() {
    let (cmd, name) = expand_shortcut("deno:serve").unwrap();
    assert_eq!(cmd, "deno task serve");
